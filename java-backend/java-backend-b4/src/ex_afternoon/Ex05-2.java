...
public class bai5
{
    public static void main(String[] args) {
        ...
        try{
            ...
            System.out.print("Nhập ngày hiện tại(dd/MM/yyyy): ");
            LocalDate currentDate = LocalDate.parse(scanner.nextLine(), formatter);

            if(currentDate.isAfter(LocalDate.now())){
                System.out.println("Lỗi!. Vui lòng nhập ngày hiện tại bé hơn hoặc bằng ngày hiện tại:"+LocalDate.now().format(formatter));
                return;
            }

            if(startDate.isAfter(currentDate)){
                System.out.println("Lỗi!. Vui lòng nhập ngày bắt đầu '" +startDate.format(formatter) +"' bé hơn hoặc bằng ngày hiện tại:'"+currentDate.format(formatter)+"'");
                return;
            }
